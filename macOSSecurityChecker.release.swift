...
func parseArguments() -> (language: String, verbose: Bool, format: String, showHelp: Bool, showVersion: Bool) {
    var language = "en"
    var verbose = false
    var format = "text"
    var showHelp = false
    ...
    return (language, verbose, format, showHelp, showVersion)
}

func printHelp() {
    print("""
    macOS Security Checker v\(VERSION)
    ...
    OPTIONS:
        ...
        --lang LANG         Set language (en, fr) - default: en
    ...
    DOCUMENTATION:
        Full documentation: https://github.com/hdrapin/macOSSecurityChecker
    """)
}
...
func printVersion() {
    ...
    macOS Security Checker v\(VERSION)
    Build: \(BUILD_DATE)
    Minimum macOS: \(MIN_OS_VERSION)
    Swift: 5.5+

    Compatible with:
    \(TARGET_OS_VERSIONS.map { "  • \($0)" }.joined(separator: "\n"))
...
// MARK: - Main Execution
let (language, verbose, format, showHelp, showVersion) = parseArguments()
...
if showHelp {
    ...
    case "csv":
        // CSV output
        print("Check,Category,Status,Value,Timestamp")
